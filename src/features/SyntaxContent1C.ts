...
export default class SyntaxContent1C extends AbstractSyntaxContent {
                    ...
                    if (methodData.signature) {
                        for (const element in methodData.signature) {
                            ...
                            methodDescription =
                                ...
                                "<p><b>Синтаксис" +
                                nameSyntax +
                                ":</b></p><p class='hljs'><span class='function_name'>" +
                                descMethod +
                                "</span><span class='parameter_variable'>" +
                                methodData.signature[element].СтрокаПараметров +
                                "</span></p>";
                            let header = false;
                            for (const param in methodData.signature[element].Параметры) {
                                if (header === false) {
                                    methodDescription =
                                        methodDescription + "<p><b>Параметры:</b></p><p>";
                                    header = true;
                                }
                                const paramDescription =
                                    ...
                                    param +
                                    ":</em></b> " +
                                    methodData.signature[element].Параметры[param]
                        ...
                        const retValue = !methodData.returns
                            ? ""
                            : ": " + ret.exec(methodData.returns)[1];
                        methodDescription =
                            methodDescription +
    ...
    private fillStructureSyntax(fillStructure) {
        ...
        for (const element in bslclasses) {
            if (!added[element]) {
                const alias =
                    bslclasses[element].name_en !== "" ? " / " + bslclasses[element].name_en : "";
                classes =
                    classes +
